...
impl endpoint::Connection for ListenerConnection {
    ...
    async fn on_incoming_begin(
        ...
        // 1С:Enterprise sends Begin with remote_channel set (wrong per spec),
        // so on_incoming_begin_inner returns NotFound instead of Ok(None).
        // Treat NotFound as remotely-initiated session.
        let result = self.connection.on_incoming_begin_inner(channel, &begin);
        match result {
            Ok(Some(relay)) => {
